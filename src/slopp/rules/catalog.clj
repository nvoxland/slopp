...
{:rule :webapp-client-routes-are-served :grain :done
   ...
   {:rule :webapp-request-paths-are-served :grain :done
    :escape "fix the path to one this store declares (the finding lists them), declare the endpoint, or write the WHOLE url if it is somebody else's server — an absolute url is never reported"
