...
(defn ^:export webapp-launcher-source
  ...
  **Deliberately THIN: require, mount, bootstrap.** Everything browser-shaped —
  ...
  `slopp.webapp/mount!`, which is slopp's own `:cljs` code and carries slopp's
  tests. A fat generated entry would be a second home for browser logic in a
  namespace whose only verification is that it compiled, which is the condition
  this capability exists to end rather than to relocate.
