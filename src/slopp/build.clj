...
(defn ^:export webapp-launcher-source
  ...
  namespaces the page reaches, supply the route table its pages declare, mount
  it, and start when the document is ready.
  ...
  **The ROUTE TABLE is generated, because the pages already declare it.** Each
  `routes` row is `[pattern page-symbol]`, derived from the `:webapp/path`
  markers by `slopp.rules.webapp/page-routes`. The entry fn used to carry the
  same table in its body — a second coordinate system for one fact, and the
  decorative half was the marker, since nothing broke when the two disagreed.
  Emitted as `update … or` rather than `assoc`, so an app that DOES declare its
  own table keeps it: the generated answer is the default, not an override.

  The headless half of the same fact is `slopp.cljnx/marked-pages`, which scans
  the loaded vars. Two readers of one marker, not two declarations.

  **Deliberately THIN: require, route, mount, bootstrap.** Everything
  browser-shaped — the render loop, click delegation, popstate, reading the
  mount point — is in `slopp.webapp.dom/mount!`, the one `:cljs` namespace slopp
  ships. A fat generated entry would be a second home for browser logic in a
  namespace whose only verification is that it compiled, which is the condition
  this capability exists to end rather than to relocate. `slopp.webapp.dom`
  earns that place by being unable to branch, which is checked; a generated
  string could not be.
  ...
  **The page namespaces are REQUIRED, not merely named.** Same failure as the
  ...
  rendering bug rather than a wiring one. That obligation GREW when the table
  moved to the markers: a page namespace used to be reachable from the entry
  because the entry named the page, and now nothing in the entry does — so the
  caller must add every page's namespace to `page-nses` itself.
  ...
  **The bootstrap is a top-level `defonce`, and that is a SECURITY property.**
  ...
  no error."
  ([page-sym page-nses] (webapp-launcher-source page-sym page-nses nil))
  ([page-sym page-nses routes]
   (let [nses  (vec page-nses)
         table (str "[" (str/join "\n                   "
                                  (for [[p s] routes] (str "[" (pr-str p) " " s "]")))
                    "]")]
     (str "(ns native.client\n"
          ...
          "(defn ^:export main []\n"
          (if (seq routes)
            (str "  (dom/mount! (update (" page-sym ") :webapp/routes\n"
                 "                      (fn [t] (or t " table ")))))\n")
            (str "  (dom/mount! (" page-sym ")))\n"))
          "\n"
