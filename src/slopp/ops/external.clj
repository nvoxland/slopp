...
;; NAMESPACES, not the register: a declaration on a module that holds
    ...
    (cond
      ...
      {:error (str "a store namespace named native.client collides with the"
                   " generated browser entry. slopp writes main and bootstrap"
                   " for a ^:web/page app — mounting it and starting it from a"
                   " top-level form — so an app of that name would be"
                   " overwritten, and any OTHER hand-written entry that calls"
                   " dom/mount! would mount a second time over the same"
                   " element. Delete yours; that is what this generates.")}
      ...
      (let [unpruned (clear-source-roots! target)
              ;; THE BROWSER ENTRY, on the same principle as the cli launcher
              ;; below and for a sharper reason. A cli app's alternative is a
              ;; `-main` written once; a browser app's is TWO wirings — the
              ;; `:cljs` shell that mounts and whatever the headless driver is
              ;; handed — with a hand-written adapter between them that nothing
              ;; compares.
              ;;
              ;; `build/webapp-launcher-source` was written, tested, and cited
              ;; by `slopp.webapp.dom/mount!`'s own `^:unused-ok` as having a
              ;; caller no reference graph could see. It had no caller at all:
              ;; its cli twin is emitted below and this one never was, so every
              ;; browser app hand-wrote the two forms slopp generates for it —
              ;; two of the three `:cljs` forms the only real consumer had left
              ;; against this capability's own metric.
              ;;
              ;; Under `cljs-src/`, where a `:cljs` namespace goes, so the
              ;; ClojureScript compiler finds it and the JVM classpath does not.
              ;; NAMED in the result, because a file appearing in a tree nobody
              ;; reads is how the previous version of this went unnoticed for a
              ;; whole wave — and a consumer has to know to delete its own entry
              client-entry
              (when (capabilities/enabled? st "webapp")
                (when-let [{:keys [page closure]} (first (rules.webapp/page-rows st))]
                  (let [rel "cljs-src/native/client.cljs"
                        lf  (io/file target rel)]
                    (io/make-parents lf)
                    (spit lf (build/webapp-launcher-source page closure))
                    rel)))]
