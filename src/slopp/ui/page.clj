(ns slopp.ui.page
  ...
  **Alone in this namespace because it mints an atom**, which is `:internal`.
  ...
  (:require [slopp.ui.app :as app] [slopp.webapp :as webapp] [clojure.string :as str]))
  ...
  {:slopp/http-paths-version 1
   :paths [{:path "/" :method :get :name 'shell
           ...
           {:path "/css/style.css" :method :get :name 'stylesheet
            :handler 'slopp.ui.styles/stylesheet
            :auth :public
            :media-type "text/css"
   ...
   [{:path "/p/:slug/store"
    ...
    {:path "/p/:slug/store/form/:id"
     ...
     :calls [{:endpoint 'slopp.ui.wire.api/form :method :get :path "/api/form/:id"}
             {:endpoint 'slopp.ui.wire.api/modules :method :get :path "/api/modules"}]}
    {:path "/p/:slug/about"
...
(def config-document
  ...
  **Shapes measured off this store's own `/api/config`**, not invented — the
  ...
  One row per branch [[slopp.ui.views/config-main]] has, because a fixture of
