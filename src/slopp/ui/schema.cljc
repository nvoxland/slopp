(ns slopp.ui.schema
  ...
  Separate from `slopp.ui.views` because none of it is hiccup: [[type-label]]
...
(defn rows
  ...
  **An `:or` opens into NUMBERED branches** — `option 1`, `option 2` — each
  ...
  existed. One did: `slopp.ui.hub/register!` answers `[:or …]` and its own API
...
(defn effectful?
  ...
  **Not from `:effectful?`, which no slopp server publishes.** This app asked
  ...
  first click. Every test passed, because `slopp.ui.page/contract-document`
