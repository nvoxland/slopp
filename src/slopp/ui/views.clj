(ns slopp.ui.views
  ...
  `slopp.ui.registry` is JVM-only (it formats a hash), so a view that required
  ...
  arrive with the SPA." (:require [clojure.string :as str] [slopp.ui.nsfilter :as nsfilter] [slopp.ui.basepath :as basepath] [slopp.ui.importance :as importance] [slopp.ui.callgraph :as callgraph] [slopp.ui.graph :as graph] [slopp.ui.schema :as schema] [slopp.ui.markdown :as markdown] [slopp.ui.metrics :as metrics] [slopp.ui.wire.api :as api]))
...
(def display-options
  ...
  namespace on arrival is its SURFACE. `slopp.ui.client.app` opened with two
...
(def tier-badges
  ...
  would have marked all 23 forms in `slopp.ui.hub` or none of them, which is
...
(def form-badges
  ...
  **It is `effectful?` and deliberately NOT the purity tier**, which is what
  ...
  `slopp.ui.hub` or none of them — the exact opposite of letting the effectful
...
(defn option-for
  ...
  than in `slopp.ui.client.app` is what makes the round trip an ordinary
...
(defn form-rail
  ...
  **This is the EGO half of `ego re-center + path spine`** — the plan's
  ...
  through [[slopp.ui.callgraph/ego]]: ranked, capped, and honest about what it
...
(defn module-graph
  ...
  positions come from `slopp.ui.graph/picture-of`, and appearance is class
  ...
  **Box LABELS drop the root every module shares**, which is named once on a
  ...
  reason: `slopp.ui.` on eleven boxes is eleven copies of the fact a reader
...
(defn gap-step
  ...
  **Absolute, not ranked.** [[slopp.ui.importance/steps]] ranks within a
...
(defn module-table
  ...
  Ordered by [[slopp.ui.graph/reading-order]] — the band, then each layer from
...
(defn code-index-main
  ...
  over the wire: `client.app` derives it with `slopp.ui.graph/picture-of` on
...
(defn form-main
  ...
  **A gap is stated, never rendered as silence.** No docstring, no recorded
  ...
  `/store/form/<id>/through/<other>` — a set because [[slopp.ui.callgraph/spine]]
...
(defn module-nav
  ...
  **The shared root is printed ONCE, as a heading.** `slopp.ui.` on all
...
(defn search-box
  ...
  **A plain GET form rather than an `:on` handler.** [[slopp.ui.app/act]] is
...
(defn try-panel
  ...
  **What was SENT is rendered beside what came back.** A response with no
  ...
  assembled: `slopp.ui.schema/request-for` stops short of a string because
...
(defn doc-link
  ...
  **The meaning of a target is entirely this app's**, and slopp says so: nothing
  ...
  Built through [[slopp.ui.schema/handler-source-path]] rather than by
...
(defn current-lens
  ...
  **A trailing segment is a lens only if [[lenses]] declares it for THIS
  ...
  The query string comes off FIRST, through [[slopp.ui.basepath/split-query]].
...
(defn with-store-picture
  ...
  **Laid out once per render now, and it used to be once per ANSWER.** It was a
  ...
  takes no `:derive`, so the page applies it — see [[slopp.ui.pages/code-page]].
...
(defn page-params
  "The ADDRESS captures for content page `page` — `{:ns \"slopp.ui.hub\" :name \"shell\"}`
...
(defn page-address
  ...
  `/pages/slopp.ui.shell/shell` — or nil when it has no address.
  ...
  It deliberately echoes `slopp.ui.schema/handler-source-path`'s
...
(defn page-main
  ...
  `address` is `{:ns \"slopp.ui.hub\" :name \"shell\"}`, the var rather than the
...
(defn routed-address
  ...
  **The translation seam for slopp's anonymous wildcards.** A route ending in
  ...
  and `slopp.ui.app/try-request`, which must select the same endpoint the page
...
(defn webapp-page-params
  "The ADDRESS captures for webapp page `row` — `{:ns \"slopp.ui.pages\" :name \"code-page\"}`
...
(defn webapp-page-address
  ...
  `/webapp/pages/slopp.ui.pages/code-page` — or nil when it has no address.
...
(defn webapp-page-main
  ...
  `address` is `{:ns \"slopp.ui.pages\" :name \"code-page\"}`, the var rather than
...
(defn dashboard-main
  ...
  Takes the documents [[slopp.ui.pages/dashboard-page]] asks for —
  `/api/namespaces`, `/api/timeline`, `/api/cost?by=ask` and
  `/api/cost?by=commit-point` — and does no arithmetic of its own. The counting
  is [[slopp.ui.metrics]], so an assertion about a number does not have to go
