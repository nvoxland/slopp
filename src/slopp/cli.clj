(ns slopp.cli
  ...
  injects the streams, renders the returned value and sets the exit code.

  **A command RETURNS DATA.** That is the decision everything else follows
  from: it makes a command an ordinary in-image `=` on a map, so the
  interesting cases — a wrong flag, a missing argument, a non-zero exit — are
  assertable with no process and no captured text. `:cli/in` / `:cli/out` are
  on the context for the two things a return value cannot express, reading
  stdin and reporting progress while work is still happening.
  ...
  **It mirrors `slopp.web` deliberately.** `context` builds an invocation from
  ...
  Neighbours: `slopp.cli.spec` decides what a command line MEANS and this
  performs it; `slopp.clidev` is the dev-side driver that runs a command
  headlessly and is not shipped."
...
(defn ^:export
  ...
  process streams. A command never touches them for ordinary output — it
  RETURNS data and slopp renders — so they are here for the two cases a return
  value cannot express: reading stdin, and writing progress while work is still
  happening.
...
(defn ^:export run
  ...
  Returns `{:cli/exit :cli/value :cli/out :cli/err}` rather than exiting, so a
  test and a process see the same answer and only the process acts on it. The
  generated launcher is the one place `System/exit` is called.
  ...
  **Order is the guarantee**, exactly as it is in `slopp.web.dispatch/handle!`:
  ...
  5. call the handler with `[ctx args]` and render what it returns.

  A handler's return may carry `:cli/exit` to choose its own status; without
  one a return is success. Everything else in the map is the ANSWER, and slopp
  renders it — which is what keeps a command an `=` on data rather than a
  string to be parsed back."
  [ctx argv]
  (let [{:cli/keys [commands out err]} ctx
        argv (vec argv)
        emit (fn [w s] (.write ^java.io.Writer w (str s "\n")) (.flush ^java.io.Writer w))
        done (fn [exit value]
               (cond-> {:cli/exit exit :cli/value value}
                 (instance? java.io.StringWriter out) (assoc :cli/out (str out))
                 (instance? java.io.StringWriter err) (assoc :cli/err (str err))))
        cmd  (first argv)]
    (cond
      (nil? cmd)
      (do (emit out (spec/program-help ctx)) (done 0 nil))

      (not (contains? commands cmd))
      (do (emit err (spec/unknown-command-error ctx cmd)) (done 2 nil))
      ...
      (let [command (get commands cmd)
        ...
        (cond
          (:help? parsed)
          (do (emit out (spec/help-text command)) (done 0 nil))
          ...
          :else
          (let [value ((:cli/handler command) ctx (:args parsed))]
            (emit out (spec/render value))
            (done (or (:cli/exit value) 0) value)))))))
