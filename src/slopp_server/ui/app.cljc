(ns slopp-server.ui.app
  "This application as DATA — the seam that lets one app be driven by a browser
  or by a test.

  `slopp-server.ui.spa/navigate!` already took the whole app as a map of plug-ins and
  ...
  What was not, was the MAP: it was assembled in `slopp-server.ui.client.app`, which
  ...
  (:require [slopp-server.ui.views :as views] [slopp-server.ui.schema :as schema] [slopp.webapp :as webapp] [slopp.http.endpoint :as endpoint] [slopp-server.ui.wire.api :as api]))
...
(defn try-request
  ...
  **The document is found under the page's OWN load key.** There is no `:main`
  ...
  same base, through [[slopp-server.ui.views/at-project]]. Written any other way the
...
(defn ^{:http/external-path
  ...
  **It returns the DECLARED map, and three keys it used to carry are gone.**
  ...
  [[slopp-server.ui.pages]] carries `^{:webapp/path \"…\"}`; a build reads those markers
  ...
  **`:sketch` is the one thing this migration lost.** rough.js is `:cljs` and
  ...
  [[slopp-server.ui.views/with-store-picture]]; slopp has it recorded unbuilt, on the
