(ns slopp-server.ui.importance
  "How much of a namespace each of its definitions accounts for, as a small
  ordinal a view can stain a name with.

  Its own namespace for the reason `slopp-server.ui.graph` is: this is COMPUTATION the
...
(defn weigh
  ...
  PAGE calls `slopp-server.ui.pages/chrome`, so seventeen callers are in the graph.
