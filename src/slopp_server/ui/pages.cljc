(ns slopp-server.ui.pages
  ...
  **Why this is not in [[slopp-server.ui.views]], which is where every screen used to
  ...
  **The word `pages` is overloaded in this repo and this is the FRAMEWORK's
  ...
  (:require [slopp-server.ui.views :as views]
...
(defn project-routes
  ...
  **Derived rather than kept beside, because a second copy is the bug.** Every
  href in [[slopp-server.ui.views]] is written project-relative — `/store/ns/demo.core`
...
(defn answered
  ...
  **The framework stopped rendering these and a page has to.** It owned
  ...
  in both directions, because every `*-main` in [[slopp-server.ui.views]] renders nil
...
(defn ^{:webapp/path "/p/:slug" :unused-ok "runtime-resolved entry — see landing-page"} timeline-page
  ...
  `*-main` stays in [[slopp-server.ui.views]] and stays `:pure`; the ask is why this
  half is not."
  [{:keys [params] :as page}]
...
(defn ^{:webapp/path "/p/:slug/store/table" :unused-ok "runtime-resolved entry — see landing-page"} code-table-page
  ...
  literal `\"table\"` is also in [[slopp-server.ui.views/lenses]] and in this var's
...
(defn ^{:webapp/path "/p/:slug/rest/paths/:method/**" :unused-ok "runtime-resolved entry — see landing-page"} endpoint-page
  ...
  **`:*` is TRANSLATED at one seam.** slopp's wildcards are anonymous, so this
  ...
  the framework's spelling stops at [[slopp-server.ui.views/routed-address]], which is
  also what [[slopp-server.ui.app/try-request]] uses, so the page and the request
...
(defn ^{:webapp/path "/p/:slug/dashboard"}
  ...
  **The canned fixture answers by PATH**, so under [[slopp-server.ui.page/page]] both
  ...
  are driven straight through [[slopp-server.ui.views/dashboard-main]] instead."
  [{:keys [params] :as page}]
