...
| `query_source {targets}` | Source of several named forms in one call, each row with a `:v` version stamp; a form you already hold at that version comes back as a reference. `{ns}` alone returns a small namespace whole and a big one as cards (name, signature, first doc sentence, `:v`, one example test); `full: true` dumps any namespace. |
...
as EDN (`:http/raw true`, `Content-Type: application/edn`, and
`^{:rest/media-type "application/edn"}` so a generated wrapper reads text rather
than attempting JSON). It lives in `slopp.http`, so it ships in the `slopp-web`
slim jar and any app can publish.
