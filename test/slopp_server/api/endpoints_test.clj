(ns slopp-server.api.endpoints-test
  ...
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [slopp-server.api.endpoints]
            [slopp-server.api.contracts :as contracts]
            [slopp.http :as slopp.http] [slopp-server.api.server :as server] [slopp.ops.external :as external] [slopp.ops :as ops] [cheshire.core :as json] [clojure.string :as str] [clojure.edn :as edn] [slopp.webdev.cljs :as cljs] [slopp-server.api.model :as model] [slopp.read.orient :as orient] [slopp.rest :as slopp.rest] [slopp-server.api.reads :as api.reads] [slopp.rest.paths :as rest.paths] [malli.util :as mu]))
      ...
      (let [ctx (server/context sess)
            ...
            GET (fn [uri]
                  (let [[path qs] (str/split uri #"\?" 2)
                        r (slopp.http/handle! ctx (cond-> {:request-method :get :uri path}
...
(deftest the-contract-endpoint-publishes-what-the-client-is-generated-from
  ...
  (let [doc     (rest.paths/paths-document server/served-namespaces)
    ...
    (testing "the document is versioned and lists the typed endpoints"
      (is (= #{:paths} (set (keys doc)))
          (str "the document carries nothing but its rows — no version key,"
               " because nothing branched on one: " (pr-str (keys doc))))
  ...
  (testing "each publisher serves EDN verbatim, whatever it documents"
    ...
    (let [ctx (server/context (atom {:store (store/empty-store)}))]
      ...
      ;; NO version key on any of them. Nothing branched on one — it existed
      ;; so a consumer could refuse rather than misread — and the rule that
      ...
      (doseq [uri ["/api/projects/demo/rest/paths" "/api/projects/demo/http/paths"
        ...
        (let [r (slopp.http/handle! ctx {:request-method :get :uri uri})]
          ...
          (let [body (edn/read-string (:body r))]
            (is (= #{:paths} (set (keys body)))
                (str uri " carries something besides its rows: " (pr-str body)))
            (is (= [] (:paths body))
...
(deftest ^:external a-consumer-generates-an-equivalent-client-from-the-published-contract
  ...
  ;; slopp-server.api.contracts generates, from HTTP alone, a client equivalent
  ;; to the one local generation produces from the store. If this holds, the
  ;; reviewer UI can live in its own project; if it does not, the wire format is
  ...
  (let [;; the producer DECLARES its API, because the published document now
        ...
        producer (atom {:store (store/ingest
                                (store/empty-store) 'slopp-server.api.endpoints
                                (str "(ns slopp-server.api.endpoints)\n\n"
...
(deftest the-source-endpoint-hands-back-the-id-it-resolved
  ...
  ;; `slopp.http.contract` exists to remove.
  ;;
...
(deftest every-path-parameter-is-declared-in-the-endpoints-request
  ...
  (let [doc      (rest.paths/paths-document ['slopp-server.api.endpoints])
        declared (fn [schema]
...
(deftest a-browser-apps-PAGES-reach-the-WIRE-and-not-only-the-derivation
  ;; slopp's own store has no browser app, so every end-to-end check of
  ;; `/api/webapp/paths` here can only ever exercise `[]`. That is the case a
  ...
  ;; encoding nobody checked. The two siblings have the same blind spot for
  ...
  (let [src (str "(ns shop.ui)\n\n"
    ...
    (testing "the row survives the encoder as DATA, symbols and all"
      ...
      (is (= [{:path "/things" :page 'shop.ui/things
               :doc "Every thing, listed."}]
             (:paths doc))
          (pr-str doc)))

    (testing "and the document carries nothing but its rows"
      ;; no version key, no mount, no :unreadable — each of those had a cause
      ;; and none of the causes survived
      (is (= #{:paths} (set (keys doc))) (pr-str doc)))

    (testing "a store with no pages answers the empty document"
      (let [none (server/context (atom {:store (store/empty-store)}))]
        (is (= {:paths []}
               (edn/read-string
