(ns slopp-server.ui.views-test
  ...
  (:require [clojure.test :refer [deftest is testing]]
            [slopp-server.ui.views :as views] [clojure.string :as str] [slopp-server.ui.styles :as styles] [slopp-server.ui.callgraph :as callgraph] [slopp-server.ui.schema :as schema] [slopp.cljnx :as cljnx] [slopp.webapp :as webapp] [slopp-server.ui.app :as app] [slopp-server.ui.wire.api :as api] [slopp.http.endpoint :as endpoint] [slopp-server.ui.pages :as pages]))
...
(deftest a-definition-is-coloured-by-category-and-badged-for-its-modifiers
  ...
  (let [v (views/ns-outline-main
    ...
    (testing "EXPORTED and EFFECTFUL are two facts and get two marks — `^:export`
              ...
              `slopp-server.ui.hub` or none — the exact opposite of letting the
...
(deftest the-tier-is-badged-on-the-NAMESPACE-because-that-is-its-grain
  ;; `module_purity` scopes by namespace PATH, so every form in slopp-server.ui.hub
  ...
  (let [page (fn [ep] (views/endpoint-main [ep] (views/endpoint-params ep) nil))
    ...
    (testing "with a published handler the page links that form's BODY"
      (let [v (page {:method :get :path "/api/modules" :name 'modules
                     :handler 'slopp-server.api.endpoints/modules
                     :request nil :response :string})]
        (is (href v "/store/source/slopp-server.api.endpoints/modules"))
        (is (re-find #"read its source" (text v)))
        (is (re-find #"slopp-server\.api\.endpoints/modules" (text v))
...
(deftest an-effectful-endpoint-says-so-before-you-can-fire-it
  ...
  ;; `slopp-server.ui.schema/effectful?` for why that direction is the safe one and
  ...
  (letfn [(text [v] (str/join " " (filter string? (nodes v))))]
    (let [data {;; the LANDING, and the only screen here that is not inside a
                ...
                :http-paths {:paths [{:path "/css/style.css" :method :get :name 'stylesheet
                                      :handler 'slopp-server.ui.styles/stylesheet :auth :public
                ...
                :webapp-pages {:paths [{:path "/p/:slug/store"
                                        ...
                                        :doc "The Code index."
                                        :calls [{:endpoint 'slopp-server.ui.wire.api/modules
                                                 :method :get :path "/api/modules"}]}
                                       {:path "/p/:slug/about"
...
(deftest an-endpoints-documentation-is-MARKDOWN-and-keeps-its-structure
  ;; The instance: Nathan opened `/p/slopp-server.ui/endpoints/get/api/projects` and
...
(deftest a-content-page-is-addressed-by-its-VAR-not-by-its-path
  ...
  ;; `/pages/*path` — and `slopp-server.ui.shell/shell` is served at `/`, so its address
  ...
  (let [shell {:path "/" :method :get :name 'shell
               :handler 'slopp-server.ui.shell/shell :auth :public
    ...
    (testing "the captures are the var's namespace and name"
      (is (= {:ns "slopp-server.ui.shell" :name "shell"} (views/page-params shell)))
      (is (= {:ns "slopp-server.ui.styles" :name "stylesheet"} (views/page-params css))))
    ...
    (testing "and a page served at the ROOT is addressable, which is the whole
      ...
      (is (= "/http/paths/slopp-server.ui.shell/shell" (views/page-address shell)))
      (is (= "/http/paths/slopp-server.ui.styles/stylesheet" (views/page-address css))))
    ...
    (testing "and the predicate finds exactly the row an address names"
      ...
      (is (not ((views/at-page? {:ns "slopp-server.ui.hub" :name "nope"}) shell))
...
(deftest the-pages-section-is-an-index-and-a-page-per-CONTENT-form
  ...
  (let [shell {:path "/" :method :get :name 'shell
    ...
    (testing "every row links to that page's own screen, addressed by the var"
      (is (some #(and (vector? %) (= :a (first %))
                      (= "/http/paths/slopp-server.ui.shell/shell" (:href (second %))))
    ...
    (testing "it names the form that serves it and links to the source, which
      ...
      (let [v (views/page-main [shell css] (views/page-params css))]
        (is (re-find #"slopp-server\.ui\.styles/stylesheet" (text v)))
        (is (some #(and (vector? %) (= :a (first %))
                        (= "/store/source/slopp-server.ui.styles/stylesheet"
                           (:href (second %))))
                  (nodes v)))))
...
(deftest the-pages-section-is-reachable-and-carries-its-own-rail
  ...
  (let [pages [{:path "/" :method :get :name 'shell
               ...
               {:path "/css/style.css" :method :get :name 'stylesheet
                :handler 'slopp-server.ui.styles/stylesheet :auth :public :doc nil
                :media-type "text/css" :shape :text :bytes 14263}]
        view (fn [screen params]
    ...
    (testing "both addresses route, and to their own subjects"
      ...
      (is (= [:http-path {:ns "slopp-server.ui.shell" :name "shell"}]
             (routed "/http/paths/slopp-server.ui.shell/shell"))))
    ...
    (testing "the RAIL lists every page on BOTH screens, with the open one
      ...
      (doseq [[screen params] [[:http-paths {}]
                               [:http-path {:ns "slopp-server.ui.styles" :name "stylesheet"}]]]
        (let [rail (views/find-region (view screen params) :nav/local)]
          (is (some? rail) (str "no rail on " screen))
          (is (re-find #"/css/style\.css" (text rail)))
          (is (re-find #"/" (text rail)))))
      (let [rail (views/find-region (view :http-path {:ns "slopp-server.ui.styles" :name "stylesheet"})
                                    :nav/local)]
        (is (some #(and (vector? %) (map? (second %))
                        (= "active" (:class (second %)))
                        (= "/p/demo/http/paths/slopp-server.ui.styles/stylesheet" (:href (second %))))
                  (nodes rail))
...
(deftest the-pages-screens-classes-and-ids-all-have-rules
  ...
  (let [pages [{:path "/" :method :get :name 'shell :handler 'slopp-server.ui.shell/shell
        ...
        classes (->> [(views/pages-main pages)
                      ...
                      (views/page-nav pages "/pages/slopp-server.ui.shell/shell")]
                     (mapcat of)
                     (keep :class)
                     (mapcat #(str/split % #"\s+"))
                     (remove str/blank?)
                     set)
        ids     (->> [(views/page-nav pages "/pages/slopp-server.ui.shell/shell")]
...
(deftest the-webapp-section-is-an-index-of-every-SCREEN-a-project-declares
  ...
  (let [rows [{:path "/p/:slug/store"
              ...
              {:path "/p/:slug/about" :page 'slopp-server.ui.pages/about-page :doc nil}]
    ...
    (testing "every declared screen is listed, addressed by its VAR"
      ...
      (is (contains? (hrefs v) "/webapp/pages/slopp-server.ui.pages/code-page"))
      (is (contains? (hrefs v) "/webapp/pages/slopp-server.ui.pages/form-page")))
...
(deftest a-webapp-page-screen-says-what-it-answers-and-what-it-fetches
  (let [rows [{:path "/p/:slug/store"
               ...
               :calls [{:endpoint 'slopp-server.ui.wire.api/modules :method :get :path "/api/modules"}]}
              {:path "/p/:slug/about" :page 'slopp-server.ui.pages/about-page :doc nil}]
        at   (fn [nm] (views/webapp-page-main rows {:ns "slopp-server.ui.pages" :name nm}))
        text (fn [t] (str/join " " (filter string? (nodes t))))
        hrefs (fn [t] (set (keep #(when (map? %) (:href %)) (nodes t))))]
    ...
    (testing "and names the var that answers it, with a way into the code —
      ...
      (is (contains? (hrefs (at "code-page"))
                     "/store/source/slopp-server.ui.pages/code-page")))
    ...
    (testing "an address naming no page degrades to a panel that names it,
      ...
      (let [v (views/webapp-page-main rows {:ns "slopp-server.ui.pages" :name "nope"})]
...
(deftest the-webapp-screens-classes-and-ids-all-have-rules
  ...
  (let [rows [{:path "/p/:slug/store" :page 'slopp-server.ui.pages/code-page
               :doc "One. Two."
               :calls [{:endpoint 'slopp-server.ui.wire.api/modules :method :get :path "/api/modules"}
                       {:endpoint 'slopp-server.ui.wire.api/form :method :get :path "/api/form/:id"}]}
              {:path "/p/:slug/about" :page 'slopp-server.ui.pages/about-page}]
        of   (fn [v] (->> (nodes v)
                          (filter #(and (vector? %) (map? (second %))))
                          (map second)))
        classes (->> [(views/webapp-pages-main rows)
                      (views/webapp-page-main rows (views/webapp-page-params (first rows)))
                      (views/webapp-page-nav rows "/webapp/pages/slopp-server.ui.pages/code-page")]
                     (mapcat of)
                     (keep :class)
                     (mapcat #(str/split % #"\s+"))
                     (remove str/blank?)
                     set)
        ids     (->> [(views/webapp-page-nav rows "/webapp/pages/slopp-server.ui.pages/code-page")]
                     (mapcat of) (keep :id) set)
        css     styles/stylesheet]
