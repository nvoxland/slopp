(ns slopp.daemon.hooks-test
  "Tests for slopp.daemon.hooks: the system-turn rule, the Bash verdicts and their JSON, the per-session cooldown, the CLI frame and the calls it becomes (JSON and EDN arguments, add/replace/change), the tail context lines and the prompt answer's assembly and cap"
  (:require [clojure.test :refer [deftest is testing]]
            [cheshire.core :as json]
            [slopp.daemon.hooks :as hooks]))
