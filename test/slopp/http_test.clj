...
(deftest ^{:bare-throw-ok "the generic-500 branch is DEFINED by a throw that is not an ex-info carrying :http/status"}
  ...
  ;; :http/status becomes that status, any other throw a generic 500. The
  ;; resolve reduce once sat OUTSIDE the try that maps a handler's or read's
  ;; throw, so a resolver's 404 escaped handle! uncaught and the adapter
  ;; answered 500 — the one-surface collapse's per-project reader resolver
  ;; needs its 404 to land as a 404.
  (let [ctx (fn [resolver]
              {:http/routes [{:method :get :path "/t/:id" :kind :rest :auth :public
                              :handler (fn [_] {:status 200 :body {:ok true}})
                              :http/resolve {:session [:open/tenant [:path-params :id]]}}]
               :http/read-performers {:open/tenant resolver}
               :http/effect-performers {} :http/perform-ctx {}})]
    (testing "a resolver throwing :http/status maps to that status, message in the body"
      (let [r (slopp.http/handle!
               (ctx (fn [_ _] (throw (ex-info "no such tenant" {:http/status 404}))))
               {:request-method :get :uri "/t/nope"})]
