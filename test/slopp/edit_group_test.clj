    ...
    (try
      ...
      (let [verifies (fn [] (count (filter #(= :verify (:op %)) (ops/journal sess))))
        ...
        (testing "both forms live in the image"
          (is (= [7] (ops/query-eval sess "(gdemo/evaluate [1 :+ 2 :* 3])")))))
  ...
  (let [sess (external/open!)]
    (try
      ...
      (let [r (ops/edit-group!
        ...
        (is (= [10] (ops/query-eval sess "(gdemo/new-helper 5)")))
        (is (= [nil] (ops/query-eval sess "(resolve 'gdemo/old-helper)"))))
      (finally (ops/close! sess)))))
