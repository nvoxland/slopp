(ns slopp.ui.pages-test
  "What the ASKING half does — [[slopp.ui.pages]].
  ...
  (:require [clojure.string :as str]
            ...
            [slopp.ui.pages :as pages]
            [slopp.ui.views :as views]
            [slopp.ui.wire.api :as api]
            [slopp.http.endpoint :as endpoint]
            [slopp.webapp :as webapp] [slopp.cljnx :as cljnx] [malli.core :as m] [slopp.ui.page :as page]))
...
(deftest ^:external every-canned-answer-satisfies-its-published-contract
  ...
  ;; wrong in production.
  ;;
  ;; **It moved here when `slopp.ui.client.check-map` was deleted**, and the
  ...
  (let [descriptors (->> (ns-publics 'slopp.ui.wire.api)
                         vals
                         (map deref)
