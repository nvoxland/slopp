...
| Small change INSIDE a big form | a `:patch` step — `{action patch, ns, name, replace [{match source} …]}`: each entry matches one subform, or a CONTIGUOUS RUN of siblings (two body forms, three clauses, a whole pair) — inside a map, binding vector or case/cond clause a run must cover whole pairs and start on a boundary; the match must PARSE on its own (complete forms, never a fragment that opens a delimiter it doesn't close), and a span that matches in two places is ambiguous and says so; a missed match returns `:source-now` (correct + resend); `text: true` for strings/docstrings; `where: {key value}` addresses the unique MAP containing those entries (registry rows — no exact text needed) |
