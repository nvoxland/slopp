...
| `slopp.git` | P4-m8 git compatibility: the PROJECTION over one bare JGit repo persisted as a cache at `.slopp/git-cache` (G3-revised) (deterministic shas, `git_map` pinning, journal→commit projection, grafting onto `git-base-sha`). Exists to be PUSHED — serving it to a git client as a remote was removed |
